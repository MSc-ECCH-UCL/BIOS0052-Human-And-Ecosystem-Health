...
btc %>%
  ...
  ggplot() + 
  geom_point(aes(YearFirstRecord, HostOrder, fill=Discovered, size=Discovered), 
             pch=21, alpha=0.8) + 
  theme_bw() +
  scale_fill_viridis_c(option="magma", direction=-1, 
...
btc %>%
  dplyr::mutate(HostOrder = factor(HostOrder, levels=order_order$HostOrder, ordered=TRUE)) %>%
  ggplot() + 
  geom_point(aes(YearFirstRecord, HostOrder, fill=Discovered, size=Discovered), pch=21, 
             alpha=0.8) + 
  theme_bw() +
...
# visualise as points
btc %>%
  dplyr::mutate(VirusFamily = factor(VirusFamily, levels=order_order$VirusFamily, 
                                     ordered=TRUE)) %>%
  ggplot() + 
  geom_point(aes(YearFirstRecord, VirusFamily, fill=Discovered, size=Discovered), 
             pch=21, alpha=0.8) + 
  theme_bw() +
  scale_fill_viridis_c(option="magma", direction=-1, 
                       begin=0.2, name="Reported number of new\nhost-virus relationships") +
